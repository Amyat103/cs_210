...
/* Header files
 ...
 call to 'cxor(result, cmd->second)' the compiler will generate the
 assembly to put result and the cmd->second field into the right
 registers and also generate `call cxoor` after using the assembler the
 resulting opcodes will be in the ccalc.o.

 Given that we have a separate cxor.c that when compiled and assemble
 will produce the opcodes for the cxor function into the cxor.o.  We
...
// The Lower and ATOQ commands uniquely treat the second argument as a
...
// The array xor and array commands uniquely uses an extra field
...
// The list xor and list commands use the second value as a pointer to
...
main(int argc, char **argv, char **envp)
{
  ...
  for (cmd = &CALC_DATA_BEGIN;  // initialize the cmd variable with the address
	...
	result = carrayxor(result, acmd->len, acmd->array);
	acmd++; 
	cmd = (void *)acmd;
      }
      break;
	...
	result = clistxor(result, lcmd->head);
	lcmd++;
	cmd = (void *)lcmd;
      }
      break;
    case 'r':
      {
	void *head = clistxor_read();
	result = clistxor(result, head);
	clistxor_free(head);
	cmd++;
      }
      break;
...
cmd2func(char c) {
  ...
  case '^':
    ...
    break;
  case 'U':
    func = (calc_simple_func_ptr)&clower;
    break;
  default:
...
writeResults(long long result) {
  if (write(1, &result, sizeof(result)) != sizeof(result)) {
    printf(stderr, "ERROR: write results failed\n");
  }
  // add the rest of the writes of the data to standard out
  // force crash -- replace with your code here

    // replace error with fail message
  if (write(1, &SUM_POSITIVE, sizeof(SUM_POSITIVE)) != sizeof(SUM_POSITIVE)) {
      printf(stderr, "ERROR: write results failed for SUM_POSITIVE\n");
  }

  if (write(1, &SUM_NEGATIVE, sizeof(SUM_NEGATIVE)) != sizeof(SUM_NEGATIVE)) {
      printf(stderr, "ERROR: write results failed for SUM_NEGATIVE\n");
  }

  size_t gap_size = (char *)&CALC_DATA_END - (char *)&CALC_DATA_BEGIN;
  if (write(1, &CALC_DATA_BEGIN, gap_size) != gap_size) {
    printf(stderr, "ERROR: write results fail for between calc_data_begin and end\n");
  }
}
